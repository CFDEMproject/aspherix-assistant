...
Picking the right `simulate` mode for how particles were inserted, verifying a `pack` insertion actually hit its target, and reaching a high cumulative target without retrying `pack`.

...
Every event a script triggers (a mark, a final compress, any one-shot state change) must happen before the last dump that's meant to capture it.
See `strategies/STRATEGIES.md`'s "Verify a per-particle-state command actually worked" entry.
